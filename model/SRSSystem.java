...
// 預設為 0，保留給使用者之後手動填入偏移量。
// 兩組表：一組給 S/Z/J/L/T，另一組給 I；O 不使用踢牆表。
// 表索引共 8 種 from→to 旋轉情況：
...
public final class SRSSystem {
	...
	private static final int[][][] KICKS_JLSTZ = new int[8][5][2];
	private static final int[][][] KICKS_I = new int[8][5][2];
	...
	// 允許手動設定某一組偏移（供使用者之後填入值）
	public static void setKick(boolean iPiece,
							   int index,    // 0..7
							   int attempt,  // 0..4
							   int dx, int dy) {
		if (index < 0 || index >= 8 || attempt < 0 || attempt >= 5) return;
		int[][][] table = iPiece ? KICKS_I : KICKS_JLSTZ;
		table[index][attempt][0] = dx;
		table[index][attempt][1] = dy;
	}

	// 內部：計算 from→to 對應的索引，CW 前四，CCW 後四
	private static int indexFor(int from, int to) {
		int cwTo = (from + 1) & 3;
		int ccwTo = (from + 3) & 3; // 等價於 from - 1（mod 4）
		if (to == cwTo) {
